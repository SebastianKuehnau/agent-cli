#!/usr/bin/env bash
# Build the self-contained single-file agent-task distribution artifact.
#
# bin/agent-task + lib/*.sh stay the source of truth for development and
...
#
# No text surgery beyond dropping bin/agent-task's own `source
...
#
# Usage: scripts/build-bundle.sh > agent-task
...
grep -v '^source "\$AGENT_LIB_DIR/' "$ROOT/bin/agent-task"
